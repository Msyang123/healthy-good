...
//    @GetMapping("/custom-plans/{id}")
    ...
    public  ResponseEntity create(@RequestBody CustomPlan customPlan) {
        log.debug("添加定制计划\t param:{}", customPlan);

        Tips tips = customPlanService.addCustomPlan(customPlan);
        if (tips.err()) {
