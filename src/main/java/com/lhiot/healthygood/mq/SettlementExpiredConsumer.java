...
//                    today.getTime() - settlementApplication.getCreateAt().getTime() > (3 * 60 * 1000)) //测试3分钟
                    .map(SettlementApplication::getId).collect(Collectors.toList());
            //修改状态
        ...
        //每天循环调用一次
//        HealthyGoodQueue.DelayQueue.SETTLEMENT_EXPIRED.send(rabbitTemplate,"nothing",5 * 60 * 1000); // 测试5分钟调一次
        HealthyGoodQueue.DelayQueue.SETTLEMENT_EXPIRED.send(rabbitTemplate,"nothing",1 * 24 * 60 * 60 * 1000 * 1);
